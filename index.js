...
app.get("/config/:env", function (req, res) {
    ...
    if (configCache.has(reqConfigEnv)) {
        ...
        // Send the complete configuration for each endpoint
        res.json(healthCheckEndpoints.map((endpoint) => ({ ...endpoint })));
    } else {
...
app.listen(process.env.PORT || 5000, () => {
    ...
    log(chalk.blue.bgRed.bold("API Health Monitor running at: http://localhost:5000"));
